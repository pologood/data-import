if (!('ace' in window))
	window['ace'] = {}
jQuery(function() {
	// at some places we try to use 'tap' event instead of 'click' if jquery
	// mobile plugin is available
	window['ace'].click_event = $.fn.tap ? "tap" : "click";
});

(function($, undefined) {
	var multiplible = 'multiple' in document.createElement('INPUT');
	var hasFileList = 'FileList' in window;// file list enabled in modern
											// browsers
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		this.$element.parent().find(':not(input[type=file])').remove();// remove
																		// all
																		// except
																		// our
																		// input,
		...
		this.$element.after('<label class="file-label" data-title="'
				...
				+ (this.settings.no_icon ? '<i class="' + this.settings.no_icon
						+ '"></i>' : '')
				+ '</span></label>'
				+ (remove_btn ? '<a class="remove" href="#"><i class="'
						+ this.settings.icon_remove + '"></i></a>' : ''));
		this.$label = this.$element.next();

		this.$label.on('click', function() {// firefox mobile doesn't allow
											// 'tap'!
			if (!this.disabled && !self.element.disabled
	...
	Ace_File_Input.prototype.show_file_list = function($files) {
		var files = typeof $files === "undefined" ? this.$element
				.data('ace_input_files') : $files;
		...
		for (var i = 0; i < files.length; i++) {
			var filename = typeof files[i] === "string" ? files[i] : $
					.trim(files[i].name);
			...
			} else if ((/\.(mpe?g|flv|mov|avi|swf|mp4|mkv|webm|wmv|3gp)$/i)
					.test(filename))
				fileIcon = 'icon-film';
			...
			if (!this.well_style)
				this.$label.find('.file-name').attr({
					'data-title' : filename
				}).find('[class*="icon-"]').attr('class', fileIcon);
			else {
				this.$label.append('<span class="file-name" data-title="'
						+ filename + '"><i class="' + fileIcon
						+ '"></i></span>');
				var type = $.trim(files[i].type);
				var can_preview = hasFileReader
						&& this.settings.thumbnail
						&& ((type.length > 0 && type.match('image')) || (type.length == 0 && fileIcon == 'icon-picture'))// the
																															// second
																															// one
																															// is
																															// for
				...
				if (can_preview) {
					var self = this;
					$.when(preview_image.call(this, files[i])).fail(
							function(result) {
								// called on failure to load preview
								if (self.settings.preview_error)
									self.settings.preview_error.call(self,
		...
		$(dropbox).on('dragenter', function(e) {
			...
			e.stopPropagation();
				...
				function(e) {
					...
					var files = dt.files;
					if (!self.multi && files.length > 1) {// single file
															// upload, but
															// dragged multiple
															// files
						var tmpfiles = [];
					...
					if (self.settings.before_change)
						ret = self.settings.before_change.call(self.element,
								files, true);// true means files have been
												// dropped
					if (!ret || ret.length == 0) {
						return false;
					}
					...
					if (ret instanceof Array
							|| (hasFileList && ret instanceof FileList))
						files = ret;

					self.$element.data('ace_input_files', files);// save
																	// files
																	// data to
																	// be used
																	// later by
					...
					self.$element.triggerHandler('change', [ true ]);// true
																		// means
																		// inner_call
					return true;
				});
	...
	var handle_on_change = function() {
		...
		if (this.settings.before_change)
			ret = this.settings.before_change
					.call(this.element, this.element.files
							|| [ this.element.value ]/* make it an array */,
							false);// false means files have been selected, not
									// dropped
		if (!ret || ret.length == 0) {
			if (!this.$element.data('ace_input_files'))
				this.reset_input_field();// if nothing selected before, reset
											// because of the newly unacceptable
											// (ret=false||length=0) selection
			return false;
		}

		// user can return a modified File Array as result
		var files = !hasFileList ? null : // for old IE, etc
		((ret instanceof Array || ret instanceof FileList) ? ret
				: this.element.files);
	...
	var preview_image = function(file) {
		var self = this;
		var $span = self.$label.find('.file-name:last');// it should be out of
														// onload, otherwise all
														// onloads may target
														// the same span because
														// of delays
		...
		reader.onload = function(e) {
			...
			$(img)
					.one(
						...
						// for example when a file has image extenstion, but
	...
	var get_thumbnail = function(img, size, type) {
		...
		try {
			...
			dataURL = canvas
					.toDataURL(/* type == 'image/jpeg' ? type : 'image/png', 10 */)
		} catch (e) {
			dataURL = null;
		}

		// there was only one image that failed in firefox completely randomly!
	...
	$.fn.ace_file_input = function(option, value) {
		...
		var $set = this.each(function() {
			...
			if (!data)
				$this.data('ace_file_input', (data = new Ace_File_Input(this,
						options)));
...
(function($, undefined) {
	$.fn.ace_spinner = function(options) {
		...
		// when min is negative, the input maxlength does not account for the
		// extra minus sign
		this
				.each(function() {
					...
					var $parent_div = $(this).closest('.ace-spinner').spinner(
							options).wrapInner(
					...
					$(this)
							.on(
									...
									function(event) {
										var delta = event.originalEvent.detail < 0
												|| event.originalEvent.wheelDelta > 0 ? 1
												: -1
										$parent_div.spinner('step', delta > 0)// accepts
																				// true
																				// or
																				// false
																				// as
					...
					$parent_div.on('changed', function() {
						that.trigger('change')// trigger the input's change
												// event
					});
...
(function($, undefined) {
	$.fn.ace_wizard = function(options) {
		...
		this.each(function() {
			...
			$wizard.$prevBtn = buttons.find('.btn-prev').eq(0).on(
					ace.click_event, function() {
						$this.wizard('previous');
					}).attr('disabled', 'disabled');
			$wizard.$nextBtn = buttons.find('.btn-next').eq(0).on(
					ace.click_event, function() {
						$this.wizard('next');
					}).removeAttr('disabled');
				...
				.each(function() {
					...
					$(this)
							...
							.on(
									...
									function() {
										$(this).next().find('.btn-colorpicker')
							...
							.after(
									'<div class="dropdown dropdown-colorpicker"><a data-toggle="dropdown" class="dropdown-toggle" href="#"><span class="btn-colorpicker" style="background-color:'
											...
											+ (settings.caret ? ' dropdown-caret'
													: '')
											+ (settings.pull_right ? ' pull-right'
													: '')
											+ '">'
											+ colors
											+ '</ul></div>').next().find(
									...
									function(e) {
										...
										e.preventDefault();
										return true;// if false, dropdown won't
													// hide!
									});
			...
			<div class="tree-item" style="display:none;">\
				'
									+ ($options['unselected-icon'] == null ? ''
											: '<i class="'
													+ $options['unselected-icon']
													+ '"></i>')
									+ '\
				<div class="tree-item-name"></div>\
			</div>');
					$this
							.addClass($options['selectable'] == true ? 'tree-selectable'
									: 'tree-unselectable');
...
(function($, undefined) {
	$.fn.ace_wysiwyg = function($options, undefined) {
		...
		var color_values = [ '#ac725e', '#d06b64', '#f83a22', '#fa573c',
		...
		var toolbar_buttons = options.toolbar
		...
		this
				.each(function() {
					...
					for ( var tb in toolbar_buttons)
						if (toolbar_buttons.hasOwnProperty(tb)) {
							...
							if (typeof button == "string"
									&& button in button_defaults) {
								button = button_defaults[button];
								button.name = toolbar_buttons[tb];
							} else if (typeof button == "object"
									&& button.name in button_defaults) {
								button = $.extend(button_defaults[button.name],
										button);
							} else
								continue;
							...
							case 'font':
								...
								for ( var font in button.values)
									if (button.values.hasOwnProperty(font))
										toolbar += ' <li><a data-edit="fontName '
												+ button.values[font]
												+ '" style="font-family:\''
												+ button.values[font]
												+ '\'">'
							...
							case 'fontSize':
								...
								for ( var size in button.values)
									if (button.values.hasOwnProperty(size))
										toolbar += ' <li><a data-edit="fontSize '
												+ size
												+ '"><font size="'
												+ size
												+ '">'
							...
							case 'createLink':
								toolbar += ' <div class="inline position-relative"> <a class="btn btn-sm '
										+ className
										+ ' dropdown-toggle" data-toggle="dropdown" title="'
										+ button.title
										+ '"><i class="'
							...
							<div class="input-group">\
								...
								<span class="input-group-btn">\
									<button class="btn btn-sm '
										+ button.button_insert_class
										+ '" type="button">'
										+ button.button_insert
										+ '</button>\
								</span>\
							</div>';
								if (button.choose_file
										&& 'FileReader' in window)
									toolbar += '<div class="space-2"></div>\
							 <div class="center">\
								<button class="btn btn-sm '
											+ button.button_class
							...
							case 'foreColor':
							case 'backColor':
								toolbar += ' <select class="hide wysiwyg_colorpicker" title="'
										+ button.title + '"> ';
								for ( var color in button.values)
									toolbar += ' <option value="'
											...
											+ '</option> ';
								toolbar += ' </select> ';
								toolbar += ' <input style="display:none;" disabled class="hide" type="text" data-edit="'
										+ button.name + '" /> ';
								break;

							case 'viewSource':
								toolbar += ' <a class="btn btn-sm ' + className
										+ '" data-view="source" title="'
										+ button.title + '"><i class="'
										+ button.icon + '"></i></a> ';
								break;
							default:
								toolbar += ' <a class="btn btn-sm ' + className
										+ '" data-edit="' + button.name
										+ '" title="' + button.title
										+ '"><i class="' + button.icon
										+ '"></i></a> ';
								break;
					...
					// if we have a function to decide where to put the toolbar,
					// then call that
					...
					toolbar.find('.dropdown-menu input:not([type=file])').on(
							ace.click_event, function() {
								return false
							}).on(
							'change',
							function() {
								$(this).closest('.dropdown-menu').siblings(
					...
					toolbar.find('input[type=file]').prev().on(ace.click_event,
							function(e) {
								$(this).next().click();
							});
					...
					var speech_input;
					if (options.speech_button
							&& 'onwebkitspeechchange' in (speech_input = document
									.createElement('input'))) {
					...
					toolbar.find('a[data-view=source]').on('click',
							function(e) {
								e.preventDefault();
